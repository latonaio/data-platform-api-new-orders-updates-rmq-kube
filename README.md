# data-platform-api-orders-updates-rmq-kube

data-platform-api-orders-updates-rmq-kube は、周辺業務システム　を データ連携基盤 と統合することを目的に、API でオーダーデータを更新するマイクロサービスです。

* https://xxx.xxx.io/api/API_ORDERS_SRV/updates/

## 動作環境

data-platform-api-orders-updates-rmq-kube の動作環境は、次の通りです。  
・ OS: LinuxOS （必須）  
・ CPU: ARM/AMD/Intel（いずれか必須）  


## 本レポジトリ が 対応する API サービス
data-platform-api-orders-updates-rmq-kube が対応する APIサービス は、次のものです。

* APIサービス URL: https://xxx.xxx.io/api/API_ORDERS_SRV/updates/

## 本レポジトリ に 含まれる API名
data-platform-api-orders-updates-rmq-kube には、次の API をコールするためのリソースが含まれています。  
...
## API への 値入力条件 の 初期値
data-platform-api-orders-updates-rmq-kube において、API への値入力条件の初期値は、入力ファイルレイアウトの種別毎に、次の通りとなっています。  

## データ連携基盤のAPIの選択的コール

Latona および AION の データ連携基盤 関連リソースでは、Inputs フォルダ下の sample.json の accepter に更新したいデータの種別（＝APIの種別）を入力し、指定することができます。  
なお、同 accepter にAll(もしくは空白)の値を入力することで、全データ（＝全APIの種別）をまとめて更新することができます。  
...
以下の sample.json の例は オーダー の ヘッダデータ が更新された結果の JSON の例です。  
